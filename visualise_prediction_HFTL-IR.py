...
checkpoint_model = "checkpoints/HFTL/080000.ckpt"
savefig_dir = "PATH_TO_SAVED_FIGURES_DIR"

prediction_pkl = "prediction_pkl/hftl_run11_80k_prediction_10corners_eliminated.pkl" 
...
with slim.arg_scope(inception_utils.inception_arg_scope()):
    logits,endpoints2 = inception_resnet_v2(data_in2, 
    ...
    field_feat_norm = tf.math.l2_normalize(
                            field_feat,
                            axis=1      
                        )         
    
feat_concat = tf.concat([herbarium_feat, field_feat_norm], 0)
...
#    topN_confidence_probs = [x[2] for x in topN_low_confidence]    
    ...
    for sample, label, prob in zip(topN_confidence_samples,
        ...
        fig, axs = plt.subplots(nrows=20, ncols=12,figsize=(18, 34)) 
        ...
        axw = axs[19, 5]
        axw.text(0.5, 0.5, "(A)", horizontalalignment='center', fontweight='bold', fontsize=20, transform=axw.transAxes) 

        
        suptitle = 'HFTL Inception-ResNet-v2\nConv2d_7b_1x1 layer\n Actual class rank: ' + str(actual_class_rank) 
        plt.sca(axs[0, 10])
        plt.title(suptitle, fontweight='bold', fontsize=22, loc='right')
    
     
        plt.tight_layout()    
        plt.savefig(os.path.join(savefig_dir, "HFTL_IR_prediction_" + str(counter) + "_" + str(class_folder_main)) + "_" + current_filename + ".jpg", bbox_inches='tight')
        plt.show()
